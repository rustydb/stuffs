...
{
	volatile BufferDesc *buf, *clockbuf, *minbuf;
	int			trycounter, fifocnt;
	struct timeval min;
...
fifocnt=NBuffers;
	...
	for (;;)
	{
		...
		if (buf->refcount == 0)
		{
			...
			else
			{
				...
				clockbuf=buf;
				printf("clock: %d u:%d lo: %lf\n", clockbuf->buf_id,clockbuf->usage_count, clockbuf->loadtime.tv_sec+0.000001*clockbuf->loadtime.tv_usec);
			...
			if (min.tv_sec>buf->loadtime.tv_sec){
				min=buf->loadtime;minbuf=buf;
			}
			else if (min.tv_sec==buf->loadtime.tv_sec)
				if(min.tv_usec > buf->loadtime.tv_usec){
					min=buf->loadtime;minbuf=buf;
				}
			if(fifocnt>0)
				fifocnt--;
				...
				UnlockBufHdr(buf);
				if(minbuf==NULL)
					elog(ERROR, "some problem with FIFO");
				break;
			}
		}
	...
	//printf("===================\n");
	printf("Policy: clock: %d FIFO: %d \n", clockbuf->buf_id, minbuf->buf_id);
	//printf("===================\n");
	LockBufHdr(minbuf);
		return minbuf;
