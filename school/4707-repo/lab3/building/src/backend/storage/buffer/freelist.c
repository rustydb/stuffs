...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
    ...
    volatile BufferDesc *oldest_buf;
    int count_fifo;
	...
	*lock_held = true;
    ...
    /* Added struct for our oldest time for FIFO */
    oldest.tv_sec = INT_MAX;
    oldest.tv_usec = INT_MAX;
	...
	for (;;)
	{
		...
		if (buf->refcount == 0) {
            if(!FIFO) {
                ...
                /* Modifications start for FIFO algorithm by Russell
                 * & Gavin */

                if (oldest.tv_sec > buf->time_inserted.tv_sec) {
                    oldest     = buf->time_inserted;
                    oldest_buf = buf;
			    } else if ((oldest.tv_sec == buf->time_inserted.tv_sec) && (oldest.tv_usec > buf->time_inserted.tv_usec)) {
                    oldest     = buf->time_inserted;
                    oldest_buf = buf;
                }
                if (count_fifo > 0) {
                    ...
                    UnlockBufHdr(buf);
                    if (oldest_buf == NULL) {
                        elog(ERROR, "FIFO failed, oldest_buf was NULL");
                    }
                    break;
                }
            }
    ...
    /* Another modification by Russell Bunch & Gavin Prentice */
    LockBufHdr(oldest_buf);
    return oldest_buf;
